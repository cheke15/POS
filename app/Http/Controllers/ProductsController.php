...
class ProductsController extends BaseController
{
    ...
    public function store(Request $request)
    {
        ...
        try {
           ...
           // if type is not is_variant, add validation for variants array
            ...
            // Convertir y validar los impuestos
            $iva = $this->convertToDecimal($request->iva);
            $ieps = $this->convertToDecimal($request->ieps);
            $ish = $this->convertToDecimal($request->ish);
            \DB::transaction(function () use ($request) {
                ...
                // Asignar impuestos
                $Product->iva = $request->iva ?? 0; // Asigna el IVA
                $Product->ieps = $request->ieps ?? 0; // Asigna el IEPS
                $Product->ish = $request->ish ?? 0;

                $totalPrice = $request->price; // Precio base
                $totalPrice += $totalPrice * ($Product->iva / 100); // Agregar IVA
                $totalPrice += $totalPrice * ($Product->ieps / 100); // Agregar IEPS
                $totalPrice += $totalPrice * ($Product->ish / 100); // Agregar ISH
        ...
        try {
           ...
           // if type is not is_variant, add validation for variants array
            ...
            \DB::transaction(function () use ($request, $id) {
                ...
                $Product->iva = $request->iva ?? 0; // Asigna el IVA
                $Product->ieps = $request->ieps ?? 0; // Asigna el IEPS
                $Product->ish = $request->ish ?? 0; // Asigna el ISH

                 // Calcular el total con impuestos
                $totalPrice = $Product->price; // Precio base
                $totalPrice += $totalPrice * ($Product->iva / 100); // Agregar IVA
                $totalPrice += $totalPrice * ($Product->ieps / 100); // Agregar IEPS
                $totalPrice += $totalPrice * ($Product->ish / 100); // Agregar ISH
                $Product->total_price = $totalPrice;
    ...
    public function Get_Products_Details(Request $request, $id)
    {
        ...
        $item['ieps'] = $Product->ieps;
        $item['ish'] = $Product->ish;
