                  ...
                  <div>
                    <b-form-group label="IVA (%)">
                      <b-form-input
                        ...
                        @input="validateNumber('IVA')"
                      ></b-form-input>
                    </b-form-group>
                  </div>

                  <div>
                    <b-form-group label="IEPS (%)">
                      <b-form-input
                        type="text"
                        v-model.number="product.ieps"
                        placeholder="Ingrese el porcentaje de IEPS"
                        @input="validateNumber('ieps')"
                      ></b-form-input>
                    </b-form-group>
                  </div>

                  <div>
                    <b-form-group label="ISH (%)">
                      <b-form-input
                        type="text"
                        v-model.number="product.ish"
                        placeholder="Ingrese el porcentaje de ISH"
                        @input="validateNumber('ISH')"
                      ></b-form-input>
                    </b-form-group>
                  </div>
                </b-form-group>
...
methods: {
      ...
      const iva = parseFloat(this.product.iva) || 0;
      const ieps = parseFloat(this.product.ieps) || 0;
      const ish = parseFloat(this.product.ish) || 0;

      const totalTax = (price * (iva + ieps + ish) / 100);
  ...
  formatNumber(number, dec) {
    ...
    if (formatted.length > dec) return `${value[0]}.${formatted.substr(0, dec)}`;
  ...
  get_products_materiels() {
    axios.get("get_products_materiels")
      .then(({ data }) => {
        this.products_ing = data;
      });
  },
  ...
  SearchProduct(result) {
    if (this.materiels.length > 0 && this.materiels.some(detail => detail.code === result.code)) {
